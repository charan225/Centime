...
const HeaderComponent = () => {
  ...
  const onChangeLang = (code) => {
    ...
    const newCurrentLanguage = languages.find(
      (l) => l.code === newcurrentLanguageCode
    );
  ...
  const dropdownTitle = () => {
    return (
      <IconButton aria-label="select language" style={{ padding: 0 }}>
          ...
          <div className="float-right">
            <DropdownButton id="dropdown-basic-button" title={dropdownTitle()}>
              {languages.map(({ code, name, country_code }) => (
                <Dropdown.Item href={code} onClick={() => onChangeLang(code)}>
                  <span
                    className={`m-r12 flag-icon flag-icon-${country_code}`}
                  ></span>
                  {name}
                </Dropdown.Item>
              ))}
